...
function freeze_level!(ctx::AbstractCompiler, lvl::VirtualSparseBandLevel, pos_stop)
    ...
    push_preamble!(
        ...
        quote
            ...
            $qos_asmbld = $(lvl.ofs)[$pos_stop + 1] - $(Tp(1))
        end,
    )
        ...
        body=Thunk(;
            ...
            preamble = quote
                if $my_i_prev > 0
                    ...
                    $qos = $(ctx(idx)) - $my_i_prev + $qos_used + 1
                end
                if $qos > $qos_asmbld
                    $qos_2 = $qos_asmbld + 1
                    while $qos > $qos_asmbld
                        $qos_asmbld = max($qos_asmbld << 1, 1)
                    end
                    $(contain(ctx_2 -> assemble_level!(ctx_2, lvl.lvl, value(qos_2, Tp), value(qos_asmbld, Tp)), ctx))
                end
                $dirty = false
            end,
