...
function virtualize(
    ...
    push_preamble!(
        ...
        quote
            ...
            $tbl = $tag.tbl
            $pool = $tag.pool
            $qos_asmbld = length($tbl)
            $stop = $tag.shape
        end,
    )
    shape = value(stop, Int)
    lvl_2 = virtualize(ctx, :($tag.lvl), Lvl, tag)
    VirtualSparseDictLevel(tag, lvl_2, Ti, ptr, idx, val, tbl, pool, shape, qos_asmbld)
end
...
function freeze_level!(ctx::AbstractCompiler, lvl::VirtualSparseDictLevel, pos_stop)
    ...
    push_preamble!(
        ...
        quote
            ...
            $qos_asmbld = $(lvl.ptr)[$(ctx(pos_stop)) + 1] - 1
        end,
    )
...
function thaw_level!(ctx::AbstractCompiler, lvl::VirtualSparseDictLevel, pos_stop)
    ...
    push_preamble!(
        ctx,
        quote
            $(lvl.qos_asmbld) = $(lvl.ptr)[$(ctx(pos_stop)) + 1] - 1
        end,
    )
    lvl.lvl = thaw_level!(ctx, lvl.lvl, value(lvl.qos_asmbld))
    return lvl
end
                    ...
                    if $qos == 0
                        ...
                        if !isempty($(lvl.pool))
                            ...
                            if $qos > $qos_asmbld
                                ...
                                Finch.resize_if_smaller!($(lvl.val), $qos_asmbld)
                                Finch.fill_range!($(lvl.val), 0, $qos, $qos_asmbld)
                            end
                        end
