...
@kwdef struct Extent{Start,Stop}
    start::Start
    stop::Stop
end
...
@kwdef struct ParallelDimension{Ext,Device}
    ext::Ext
    device::Device
end

@kwdef struct VirtualParallelDimension
    ext
    device
end
...
virtual_extent(::Type, start, stop) = throw(ArgumentError("Unsupported type"))
virtual_extent(::Type{T}, start, stop) where {T<:Integer} = VirtualExtent(start, stop)
function virtual_extent(::Type{T}, start, stop) where {T<:Real}
    VirtualContinuousExtent(start, stop)
end
function virtual_extent(::Type{T}, start, stop) where {T<:Limit}
    VirtualContinuousExtent(start, stop)
end
