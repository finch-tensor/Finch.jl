...
function Finch.unfurl(
    ...
    Lookup(;
        body = (ctx, j) -> Provenance(;
            path = SubFiberOf(j),
            body = VirtualSparseMatrixCSCColumn(arr, j),
        ),
    )
end
...
function Finch.unfurl(
    ...
    Thunk(;
        ...
        body     = (ctx) -> Provenance(;
            path = SubFiberOf(qos),
            body = Finch.instantiate(ctx, Finch.VirtualSparseScalar(nothing, arr.Tv, zero(arr.Tv), gensym(), :($(arr.val)[$(ctx(qos))]), dirty), mode),
        ),
        epilogue = quote
            if $dirty
                $(arr.idx)[$qos] = $(ctx(idx))
