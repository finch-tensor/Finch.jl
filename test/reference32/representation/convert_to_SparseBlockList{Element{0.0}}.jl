quote
    ...
    tmp_lvl_i_prev = -1
    0 < 1 ||
        throw(FinchProtocolError("SparseBlockListLevels cannot be updated multiple times"))
    ...
    (
        tmp=Tensor(
            (SparseBlockListLevel){Int32}(
                tmp_lvl_2, ref_lvl.shape, tmp_lvl_ptr, tmp_lvl_idx, tmp_lvl_ofs
            ),
        ),
    )
end
