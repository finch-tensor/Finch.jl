begin
    ...
    A_lvl =
        (((((ex.bodies[1]).bodies[2]).body.bodies[2]).body.rhs.args[1]).args[1]).tns.bind.lvl
    ...
    y_lvl =
        (((((ex.bodies[1]).bodies[2]).body.bodies[2]).body.rhs.args[1]).args[2]).tns.bind.lvl
    y_lvl_ptr = y_lvl.ptr
    y_lvl_idx = y_lvl.idx
        ...
        if phase_stop >= 1
            ...
            while i <= phase_stop
                ...
                if y_lvl_i == phase_stop_2 && A_lvl_2_i == phase_stop_2
                    ...
                    p_val = (Finch.Chooser{0}())(
                        p_val, ifelse(y_lvl_2_val_2 && A_lvl_3_val_2, phase_stop_2, 0)
                    )
                    y_lvl_q += 1
                    A_lvl_2_q += 1
