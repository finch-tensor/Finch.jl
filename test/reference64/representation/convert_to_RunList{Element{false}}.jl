quote
    ...
    if phase_stop >= 1
        ...
        while true
            ...
            if ref_lvl_i < phase_stop
                ...
                if tmp_lvl_qos_3 > tmp_lvl_qos_alloc
                    tmp_lvl_qos_2 = tmp_lvl_qos_alloc + 1
                    while tmp_lvl_qos_3 > tmp_lvl_qos_alloc
                        tmp_lvl_qos_alloc = max(tmp_lvl_qos_alloc << 1, 1)
                    end
                    Finch.resize_if_smaller!(tmp_lvl_right, tmp_lvl_qos_alloc)
                    Finch.fill_range!(tmp_lvl_right, ref_lvl.shape, tmp_lvl_qos_2, tmp_lvl_qos_alloc)
                    Finch.resize_if_smaller!(tmp_lvl_val_2, tmp_lvl_qos_alloc)
                    Finch.fill_range!(tmp_lvl_val_2, false, tmp_lvl_qos_2, tmp_lvl_qos_alloc)
                end
    ...
    if tmp_lvl_qos - tmp_lvl_qos_set > 0
        ...
        tmp_lvl_qos_used = tmp_lvl_qos - 1
    end
    qos = tmp_lvl_qos_used + (tmp_lvl_i_prev < ref_lvl.shape)
    qos += 1 - 1
    if qos > tmp_lvl_qos_alloc
        Finch.resize_if_smaller!(tmp_lvl_right, qos)
        Finch.fill_range!(tmp_lvl_right, ref_lvl.shape, tmp_lvl_qos_used + 1, qos)
        pos_start = 1 + tmp_lvl_qos_used
    ...
    tmp_lvl_qos_alloc = tmp_lvl_ptr[1 + 1] - 1
    resize!(tmp_lvl_val_2, tmp_lvl_qos_alloc)
    Finch.resize_if_smaller!(tmp_lvl_val, tmp_lvl_qos_alloc)
    Finch.fill_range!(tmp_lvl_val, false, 1, tmp_lvl_qos_alloc)
    q = 1
    q_2 = 1
    for p_2 = 1:1
    ...
    tmp_lvl_qos_alloc = q_2 - 1
    resize!(tmp_lvl_val, tmp_lvl_qos_alloc)
