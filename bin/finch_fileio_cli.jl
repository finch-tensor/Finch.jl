...
function read_tensor(path_with_dataset::AbstractString)
    ...
    if !(endswith(path, ".bsp.h5") || endswith(path, ".bsp.hdf5"))
        throw(
            ArgumentError("dataset addressing is only supported for binsparse HDF5 files")
        )
    end
...
function main(args)
    ...
    elseif command == "check_canonical_equivalence"
        ...
        equivalent, message = tensor_matches_canonical(
            tensor, canonical_matrix, canonical_pattern
        )
        if equivalent
            println("OK")
            return 0
